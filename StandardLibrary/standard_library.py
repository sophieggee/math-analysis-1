...
def prob2():
    ...
    print(f"int is mutable: {int_bool},", f"str is mutable: {str_bool},",
...
def hypot(a, b):
    ...
    sum_of_squares = calculator.sum(
        a_squared, b_squared)  # a squared plus b squared
    # calculating square root of above sum (hypotenuse equation)
...
def power_set(A):
  ...
  for i in range(0, len(A)+1):  # iterates through index of passed in iterable
      # appends multiple combinations that will make up powerset
